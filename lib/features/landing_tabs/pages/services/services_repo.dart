...
class ServicesRepo extends BaseRepository {
  Future<ListResult<List<INVGroupStageModel>>?> getInvGroupStages({CachePolicy? cachePolicy}) async {
      ...
      mapper: (dynamic json) {
        if (json is Map<String, dynamic> && json.containsKey('groupDtos')) {
          return (json['groupDtos'] as List)
              .map((x) => INVGroupStageModel.fromJson(x as Map<String, dynamic>))
              .toList();
        } else if (json is List) {
          return json.map((x) => INVGroupStageModel.fromJson(x as Map<String, dynamic>)).toList();
        }
        return [];
      },
    );
